...
################################################# START SECTION OF LOADING PARAMETERS #################################################
    ...
    para = str(args.bulk)+"_data_"+str(args.sc_data)+\
        "_drug_"+str(args.drug)+"_bottle_"+str(args.bottleneck)+"_edim_"+str(args.bulk_h_dims)+\
            "_pdim_"+str(args.predictor_h_dims)+"_model_"+reduce_model+"_dropout_"+str(args.dropout)+"_gene_"+str(args.printgene)+"_lr_"+str(args.lr)+"_mod_"+str(args.mod)+"_sam_"+str(args.sampling)
    print(para)              
    ...
    out_path = log_path+now+str(args.sc_data)+".err"
    log_path = log_path+now+str(args.sc_data)+".log"
...
################################################# END SECTION OF ANALYSIS FOR BULK DATA #################################################
    ...
    roc_pret = roc_auc_score(Y_test, sens_pb_pret)
    
    report_dict = classification_report(Y_test, lb_results, output_dict=True)
    f1score = report_dict['weighted avg']['f1-score']


    report_dict2 = classification_report(Y_test, lb_pret, output_dict=True)
    f1_pre = report_dict['weighted avg']['f1-score']

    report_df['f1_score'] = f1score
    file = 'save/bulk_f'+data_name+'_f1_score_ori.txt'
    file1 = 'save/bulk_f'+data_name+'_ap_score_ori.txt'
    file2 = 'save/bulk_f'+data_name+'_roc_score_ori.txt'
    ...
    sc.pl.umap(adata,color=color_list,neighbors_key="Trans",
            save=para+"_TL.tiff",
            show=False,title=title_list)


    with open(file, 'a+') as f:
        f.write(para+"_transfer_after"+'\t'+str(f1score)+'\n') 
        f.write(para+"_transfer_before"+'\t'+str(f1_pre)+'\n') 
    with open(file1, 'a+') as f:
        f.write(para+"_transfer_after"+'\t'+str(ap_score)+'\n') 
        f.write(para+"_transfer_before"+'\t'+str(ap_pret)+'\n') 

    with open(file2, 'a+') as f:
         f.write(para+"_transfer_after"+'\t'+str(roc_score)+'\n') 
         f.write(para+"_transfer_before"+'\t'+str(roc_pret)+'\n') 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epochs', type=int, default=250,help='Number of epoches training. Default: 500')
